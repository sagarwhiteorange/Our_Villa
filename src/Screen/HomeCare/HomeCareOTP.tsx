...
enum Url {
  HomeCareNewPassword = 'HomeCareNewPassword',
}

const HomeCareOTP = (props: any) => {
  ...
  const confirmCode = async() => {
    ...
    if(response.status) {
      setVisible(true)
      setTimeout(() => {
        navigation.navigate('HomeCareNewPassword', {email: email})
      }, 3000);
    } else {
    ...
    <ScrollView>
        <View style={Styles.marginHorizontal15}>
          ...
          <View style={Styles.mb10}>
            ...
            {OTPError && <Text style={[OTPError ? Styles.mb20: null, Styles.fontBlack16,Styles.fontBook16, {color: Colors.textDanger}]}>{OTPError}</Text>}
          </View>
          <View style={[Styles.flexRow, Styles.mt20, Styles.mb20]}>
            <Text style={[Styles.fontBlack16, Styles.fontBook16]}>{Strings.ResendOTP}</Text>
            <Text style={[Styles.fontBlack16, Styles.fontBook16]}>01:25</Text>
          </View>


            <TouchableOpacity onPress={() => confirmCode()} style={Styles.btnBlue}>
            {/* <TouchableOpacity onPress={navigation.navigate('UserNewPassword')} style={Styles.btnBlue}> */}
